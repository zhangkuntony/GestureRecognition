...
def logging_csv(number, mode, landmark_list, point_history_list):
    ...
    if mode == 1 and (0 <= number <= 9):
        csv_path = 'results/keypoint.csv'
        with open(csv_path, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([number, *landmark_list])

    if mode == 2 and (0 <= number <= 9):
        csv_path = 'results/point_history.csv'
        with open(csv_path, 'a', newline='') as f:
            writer = csv.writer(f)
...
def draw_info(image, fps, mode, number):
    ...
    cv.putText(image, "FPS:" + str(fps), (10, 30), cv.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 4, cv.LINE_AA)
    ...
    while True:
        ...
        if getattr(results, 'multi_hand_landmarks', None) is not None:
            for hand_landmarks, handedness in zip(
                getattr(results, 'multi_hand_landmarks'),
                getattr(results, 'multi_handedness')
            ):
                bounding_rect = calc_bounding_rect(debug_image, hand_landmarks)
